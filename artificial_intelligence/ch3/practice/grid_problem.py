...
class RandomGridSearchProblem(util.SearchProblem):
        ...
        for action, move, cost_func in self.action_move_cost_tuples:
            ...
            if all((new_state not in self.blocks,
                ...
                if self.use_heuristic:
                    cost += self.heuristic(new_state) - self.heuristic(state)  # use self.heuristic
    ...
    def heuristic(self, state):
        row, col = state
        return self.h_row[row] + self.h_col[col]

    def print_grid(self):
        grid_str = ''
        grid_str += '# # ' + ' # '.join(map(str, range(self.num_cols))) + '\n'
...
if __name__ == '__main__':
    ...
    path, totalCostAStar, numStatesExplored = ucs.solve(
        problem, init_cost=problem.heuristic((0, 0))0) #initialize ?

    assert totalCost == totalCostAStar
